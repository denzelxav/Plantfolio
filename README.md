...
## Use case example
Scenario: Bob, a busy man with a passion for indoor plants, has difficulties to care for his plants due to his schedule. He uses the application to simplify and improve his plant care routine.
...
7. Saving data: he makes sure his data is safe by manually saving, although his data will most likely be saved automatically.


## Additional documentation for other developers (optional*2)
This project contains multiple files, most of them ordered in several folders which are listed below.
* database: this folder contains files with helper functions and classes to create the complete database of all the plants available in the application. The initial database was retrieved with an API from https://perenual.com/docs/api .
...
* docs: this folder contains the meeting notes which were made every week during a meeting with the TA of our project group. Additionally, it contains the project plan made in the first two weeks of the project along with a changelog, documenting the changes made to it while working on the project. The project plan discusses the preparations that had to be made before starting to code and the requirements that were imposed beforehand.
* test: this folder contains the testing files to test the functionality of the underlying classes of the application. Most classes from group-10/project/classes have its own corresponding test file.
* Top-level files: there are a few top-level files that do not belong to a subfolder. Most of them were given in the template and are used to 

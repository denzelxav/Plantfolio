...
def test_sunlight():
    ...
    assert maple.sunlight_score == 25 - offset*25, "sunlight score was not calculated or calculated incorrectly"
    sunny_spot = Spot("spot", Sunlight.FULL_SUN, "humid", None, 20, "room1")
    maple.change_spot(sunny_spot)
    assert maple.sunlight_score == 100 - offset*25, "sunlight score cache was not updated"
...
def test_health():
    ...
    assert maple.get_health_score() == int(100-0.45*offset*25), "health score was not calculated incorrectly"
